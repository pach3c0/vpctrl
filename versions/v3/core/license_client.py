...

5QIDAQAB
-----END PUBLIC KEY-----"""
...
class LicenseClient:
    ...
    def check_license(self) -> LicenseResult:
        ...
        Sempre tenta validação online primeiro. Só usa token local se offline.
        """
        stored_key = self._settings.value(SETTINGS_KEY_KEY, "")
        stored_token = self._settings.value(SETTINGS_TOKEN_KEY, "")

        # Step 1: Sempre tenta validar online primeiro
        if stored_key:
        ...
        # Step 2: Sem chave salva — verifica se tem token offline válido (grace)
        if stored_token:
            payload = _decode_token_offline(stored_token)
            if payload and payload.get("fingerprint") == self._fingerprint:
                if payload.get("_grace_period"):
                    return LicenseResult(
                        status=LicenseStatus.GRACE,
                        message="Running offline (grace period). Connect to internet to refresh license.",
